<template>
	<div>
		...
		<b-modal id="modal-mapas" size="md" scrollable centered hide-backdrop title="Registrar Mapa de Procesos" hide-footer>
			<mapaprocesos-editor @cerrarModal="cerrarModal"></mapaprocesos-editor>
		</b-modal>
	</div>
</template>
...
export default {
		...
		return {
			...
			fields: [
				...
				*/
				...
				{ key: 'priorizado2', label: 'Priorizado', sortable: true},
	...
	methods: {
		...
		cargarVer: function(item){
			console.log(item);
			location.href = '/mapaprocesosver/cargar/'+item.idMapaProcesos;
		},
		cargarMapas(){
			this.mostrarLoader = true;
			var that = this;
			//this.totalRows = this.items.length;
			this._axios('/mapaprocesos/listar', null, function(res){
				//console.log(res);
				res = res.data;
				if (res.mensaje == ''){
